...
def setup_logging():
    ...
    logger.info("=" * 70)
    logger.info(f"IMU Port: {config.PATHFOLLOWER_IMU_PORT} @ {config.PATHFOLLOWER_IMU_BAUD} baud")
    logger.info(f"RTK Port: {config.PATHFOLLOWER_RTK_PORT} @ {config.PATHFOLLOWER_RTK_BAUD} baud")
    logger.info(f"Feather Port: {config.PATHFOLLOWER_FEATHER_PORT} @ {config.PATHFOLLOWER_FEATHER_BAUD} baud")
